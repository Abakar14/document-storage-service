...
public class ShowStudentDetailsController implements Initializable {
	...
	@Value("${show.student.details.title}")
	...
	@FXML
	private JFXTextField registeredOnTextField;
		...
		if (student != null) {
			try {
				...
				if (!DSSValidation.isObjectNull(student.getCreatedOn())) {
					...
					registeredOnTimePicker.setDateTimeValue(student.getCreatedOn());
				}

				registeredOnTimePicker.setEditable(false);
				registeredOnTimePicker.setPrefWidth(134);

				if (registeredOnTextField == null) {
					registeredOnTextField = new JFXTextField();
				}

				registeredOnTextField.setText(DSSFormat
						.formateLocalDateTimeToString(registeredOnTimePicker.getDateTimeValue(), dateTimeFormat));
...
//	}
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		assert registeredOnTextField != null
		...
		assert capturePictureButton != null
				: "fx:id=\"capturePictureButton\" was not injected: check your FXML file 'show-student-details.fxml'.";
